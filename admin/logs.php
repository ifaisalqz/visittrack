...
$totalQuery = $conn->query("SELECT COUNT(*) FROM visitors WHERE status IN ('completed', 'rejected', 'expired')")->fetchColumn();
$completedQuery = $conn->query("SELECT COUNT(*) FROM visitors WHERE status = 'completed'")->fetchColumn();
$rejectedQuery = $conn->query("SELECT COUNT(*) FROM visitors WHERE status = 'rejected'")->fetchColumn();

// تعديل كود الـ SQL عشان نضمن إنه يجيب البيانات كلها وما يختفي أي شيء
$logs = $conn->query("SELECT * FROM visitors WHERE status IN ('completed', 'rejected', 'expired') ORDER BY id DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700/50">
                        <tr class="uppercase text-[10px] font-black tracking-widest text-slate-500"><th class="p-6">Visitor & Date</th><th class="p-6">Visit Details</th><th class="p-6">Duration</th><th class="p-6 text-center">Status</th></tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50">
                        <?php 
                        foreach($logs as $l): 
                            // حساب الوقت بأمان من خلال PHP
                            $dur_min = null;
                            $dur_sec = null;
                            if (!empty($l['actual_checkin']) && !empty($l['actual_checkout'])) {
                                $dur_sec = strtotime($l['actual_checkout']) - strtotime($l['actual_checkin']);
                                $dur_min = floor($dur_sec / 60);
                            }
                        ?>
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                            <td class="p-6"><div class="font-black text-slate-800 dark:text-white text-base mb-1"><?php echo htmlspecialchars($l['full_name']); ?></div><div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest"><?php echo date('M d, Y', strtotime($l['created_at'])); ?></div></td>
                            <td class="p-6"><div class="text-sm font-bold text-slate-700 dark:text-slate-300"><?php echo htmlspecialchars($l['purpose']); ?></div><div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Host: <?php echo htmlspecialchars($l['host_name']); ?></div></td>
                            <td class="p-6">
                                <?php if ($dur_sec !== null): ?>
                                    <div class="text-sm font-black text-slate-800 dark:text-white font-mono"><?php echo $dur_min; ?> <span class="text-[10px] text-slate-500 uppercase tracking-widest">Min</span></div>
                                    <div class="text-[10px] font-bold text-slate-500 mt-1"><?php echo $dur_sec; ?> sec total</div>
                                <?php else: ?>
                            ...
                            <td class="p-6 text-center">
                                <?php 
                                    $badgeColor = 'bg-slate-100 dark:bg-slate-500/10 text-slate-600 dark:text-slate-500 border-slate-200 dark:border-slate-500/20';
                                    if($l['status'] == 'completed') $badgeColor = 'bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-500 border-green-200 dark:border-green-500/20';
                                    if($l['status'] == 'rejected') $badgeColor = 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-500 border-red-200 dark:border-red-500/20';
                                ?>
                                <span class="inline-block px-4 py-1.5 rounded-xl border text-[9px] font-black uppercase tracking-widest <?php echo $badgeColor; ?>"><?php echo $l['status']; ?></span>
                            </td>
                        </tr>
                        <?php endforeach; if(count($logs) == 0): ?><tr><td colspan="4" class="p-12 text-center text-slate-500 text-xs font-bold uppercase tracking-widest">No history logs found</td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
        </div>
